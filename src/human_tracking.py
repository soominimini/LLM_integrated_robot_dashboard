...
class HumanTracking:
    ...
    def __init__(self, human_detector=None):
        ...
        if human_detector:
            ...
            self.human_detector.setup(kinematic_interface=self.ikin)
    ...
    def _forget_absences(self):
        self.persons_lock.acquire()
        keys_to_remove = [id for id, data in self.persons.items() if
                          time.time() - data['last_seen'] > self.ABSENCE_MEMORY_DURATION]
