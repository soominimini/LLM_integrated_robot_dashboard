...
class HumanPresenceDetection(BaseNode):
    ...
    def on_vad_trigged(self):
        ...
        head_pitch_velocity = self.kinematics.joints_state.velocity[
            self.kinematics.joints_state.name.index("HeadPitch")]
        ...
        if not self.image_queue.empty():
            ...
            for face_obj in face_objs:
                ...
                try:
                    ...
                    embedding = DeepFace.represent(img_path=roi, detector_backend=self.face_detector_backend,
                                                   model_name="VGG-Face")[0]['embedding']
                ...
                wy = int(xyz[1] * 100)
                wz = int(xyz[2] * 100)
                cv2.putText(image, f"id:{face_id} ({wx}, {wy}, {wz})", (x + 5, y + 15), cv2.FONT_HERSHEY_SIMPLEX, 0.4,
                            (0, 0, 255), 2)
